...
if (isset($_POST['btnInsert'])) {
    ...
    if ($insertResult) {
        echo "<script>alert('Thêm phiếu thành công!'); window.location.href = 'nhanvienkho.php?action=AImportInventory';</script>";
        exit;
    } else {
...
if ($viewId > 0) {
    ...
    if ($transactionResult && mysqli_num_rows($transactionResult) > 0) {
        ...
        echo '<h4 class="mb-4">Chi tiết phiếu Nhập kho #' . $t['id'] . '</h4>';
        echo '<table class="table table-bordered">';
        echo '<tr><th>Nguyên liệu</th><td>' . htmlspecialchars($t['ingredient_name']) . '</td></tr>';
        echo '<tr><th>Loại giao dịch</th><td>Nhập kho</td></tr>';
        ...
        echo '</table>';
        echo '<a href="nhanvienkho.php?action=AImportInventory" class="btn btn-secondary">Quay lại</a>';
    } else {
...
elseif ($addMode) {
    ...
    <h4 class="mb-4">Thêm phiếu Nhập kho mới</h4>

    <form method="POST" action="nhanvienkho.php?action=AImportInventory&addTransaction=true" id="importForm">
        ...
        <a href="nhanvienkho.php?action=AImportInventory" class="btn btn-danger">Hủy</a>
...
// --- HIỂN THỊ DANH SÁCH PHIẾU NHẬP ---
    ...
    $transactions = $transactionCtrl->getTransactionsByType(1);

    echo '<h2 class="mb-4">Danh sách phiếu Nhập kho</h2>';
    echo '<a href="nhanvienkho.php?action=AImportInventory&addTransaction=true" class="btn btn-success mb-3">Thêm phiếu</a>';

    // Form tìm kiếm
    echo '<form method="GET" class="row mb-4">';
    echo '<input type="hidden" name="action" value="AImportInventory">';
    ...
    echo '<div class="col-md-auto"><a href="nhanvienkho.php?action=AImportInventory" class="btn btn-danger">Hủy</a></div>';
    echo '</form>';
    ...
    if (count($filtered) == 0) {
        ...
        foreach ($filtered as $tran) {
            ...
            echo '<td><a href="nhanvienkho.php?action=AImportInventory&viewTransaction=' . $tran['id'] . '" class="btn btn-info btn-sm">Xem</a></td>';
            echo '</tr>';
        }
        echo '</tbody></table>';
    }
